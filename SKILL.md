...
向用户提出选择，询问本次建模使用的算法模式：

> 请选择您希望使用的 KIRI Engine 重建模式：
> 1. **Photo Scan**（适用于常规带有丰富表面纹理的物体与场景）
...
1. **模型名称 (Model Name)**：
   - 默认值建议为所选文件夹名或文件名，用户可自定义。
2. **转 Mesh 的导出格式 (Mesh Export Format)**：
   - 支持从以下格式中挑选（可单选或多选）：
     `OBJ`、`FBX`、`STL`、`GLB`、`GLTF`、`USDZ`、`PLY`、`XYZ`
3. **Remove Background（是否去除背景）**：
   - 选项：`是` / `否`
4. **Train AI（是否同意数据参与 AI 训练）**：
   - 选项：`同意` / `不同意`
5. **Visibility（模型可见性）**：
   - 选项：`Public (公开)` / `Private (私密)`

在 WebApp 对应的设置面板中完成相应表单项的填写与勾选。
